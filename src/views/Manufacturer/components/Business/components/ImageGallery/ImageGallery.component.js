...
const ImageGalleryComponent = ({ images, thumbnail, type, title, data, userId, form_values, image_type }) => {
  ...
  const handleDeleteImage = (type, index, uniIndex) => {
    setShowConfirm(true);
    setThumbnailIndex(index);
    const selectedImageId = remoteImages[index].id; 
    suspendItem(selectedImageId);
  }
    ...
    if (remoteImages.length < 10) {
      return (
        <File
          ...
          type={['jpg', 'png', 'jpeg']}
          placeholder={'Images'}
          max_count={10 - remoteImages.length <= 0 ? -1 : 10 - remoteImages.length}
          component={localImages.length > 0 ? (renderSaveButton()) : (renderAddButton())}
        />
      );
    }
    return null;

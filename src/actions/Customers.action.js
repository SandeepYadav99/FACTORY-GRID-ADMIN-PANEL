...
export function actionFetchCustomers(
  index = 1,
  sorting = {},
  filter = {},
  shouldReset = false
) {
  const request = serviceGetCustomers({
    index,
    row: sorting.row,
    order: sorting.order,
    ...filter,
...
export function actionChangeStatusCustomers(params) {
  const request = serviceUpdateCustomers({
    id: params.id,
    status: params.type,
  });
  return (dispatch) => {
    request.then((data) => {
      dispatch({
        type: CHANGE_STATUS,
        payload: { id: params.id, status: params.type },
      });
    });
  };
}
...
export function actionSetPageCustomers(page) {
  ...
  if (totalLength <= (page + 1) * Constants.DEFAULT_PAGE_VALUE) {
    store.dispatch(
      actionFetchCustomers(serverPage + 1, sortingData, {
        query,
